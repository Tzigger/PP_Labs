...
class Polyglot {
    ...
    private static int SumCRC(String token){
        //Aici elimin primul si ultimul caracter din sir
        token = removeFirstAndLastChar(token);
        ...
        String coeffs = Arrays.stream(coefficients).mapToObj(String::valueOf).collect(java.util.stream.Collectors.joining(", "));
        String multiLineString = "coeffs = [%s]\n" +
                "total = 0\n" +
                "for i, ch in enumerate('%s', 1):\n" +
                "    power = min(i, 5)\n" +
                "    total += coeffs[power] * ord(ch)\n" +
                "total";
        String pythonExpression = String.format(multiLineString, coeffs, token);
    ...
    public static String removeFirstAndLastChar(String inputString) {
        if (inputString == null || inputString.length() < 2) {
            throw new IllegalArgumentException("Șirul trebuie să aibă cel puțin două caractere.");
        }
